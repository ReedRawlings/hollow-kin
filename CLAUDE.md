...
- Turn-based combat (abilities, MP, buffs/debuffs, status, player-only crits); **random enemy targeting**; single-enemy auto-target. The player's root menu is **`FIGHT / MAGIC / ITEM`** — **Defend and RUN were both cut on 2026-07-30.** Defend had a full engine implementation and no way for a player to pick it; RUN was a permanently-disabled mockup leftover. Mitigation is buff stages and items; escaping is the Smoke Husk. Do not reintroduce either — see the *Removed From The Design* section of `combat-system.md`
...
- vitest test suite — 443 tests across 20 files, including roster authoring invariants in `src/data/creatures.test.ts` (every ability id resolves, every band is priced, every archetype shares one family rite) and pool invariants in `Traits.test.ts`
