...
- **Pack Tempo** as a battle-local mastery resource that becomes a persistent Relay opportunity at three points;
...
- authored **Link Arts** as the offensive reason to manipulate turn order;
...
- The player-facing root actions are `FIGHT / MAGIC / ITEM / RELAY`; Relay is disabled until Pack Tempo reaches its base cap of three.
...
- Run-scoped boons already exist and are the only run-scoped modifier layer; a boon with no battle limit is a run-long boon.
...
- The first implementation does not need the complete move, boon, Mark, Instinct, Afterform, Omen, Break, and Weave catalogs.
...
## Stable Core Revision: Persistent Relay and Link Arts

This section supersedes the earlier Relay Window prototype retained immediately below for implementation history.

### Turn and intent structure

- SPD creates the initial interleaved order from unique **action slots**, not creature identities.
- Every living creature receives one standard action slot each round.
- Exact enemy moves and targets are committed per enemy action slot and revealed at round start.
- Bosses and explicit encounter rules may grant an enemy multiple, separately telegraphed action slots.
- Player Kin receive one action per round, without exception. Allied extra action slots are not planned; Relay moves an unused action rather than adding one.
- Pulling an unused action forward removes its old timeline position. Granting an extra action creates a new slot with an explicit source.

### Pack Tempo lifecycle and generation

- Pack Tempo begins at zero, caps at three, carries between rounds, and clears at battle end.
- Reaching three makes **Relay Ready**. This state does not expire until Relay is used or the battle ends.
- While full, ordinary generation is wasted; the UI never forces the player to spend Relay.
- By default, a complete action may generate at most one Tempo even if it satisfies multiple triggers.
- Explicit traits, boons, and other authored modifiers may increase the amount generated by an action or add new triggers.
- Default generation triggers are exploiting a weakness, fully resolving an Omen, causing Break, and triggering Rebound. A weakness hit grants Tempo whether or not that weakness was previously known; knowledge controls previews and auto-combat decisions, not the reward.
- **Rebound** means an attack was actively deflected by an authored move, trait, or boon. Ordinary elemental resistance never generates Tempo.
- Basic Attack, ordinary hits, passive damage, damage-over-time, and criticals do not generate Tempo merely for occurring.

### Relay

- Relay costs all three base Tempo.
- Relay is queued from the normal command panel and resolves after the current allied action.
- Baseline Relay selects a living ally's unused standard action and moves it directly after the current slot.
- Relay never adds a baseline action, never targets a knocked-out Kin, and never crosses the battle boundary.
- Traits, boons, and encounter rules may alter starting Tempo, capacity, generation, persistence between battles, Relay cost, or legal Relay targets. These exceptions must remain visible.

### Link Arts

- Link Arts are authored ordered recipes made from broad move styles such as element, Attack, Buff, Debuff, Heal, Guard, Physical, Magical, and target scope.
- Link participants must be different Kin. Duo Links require two Kin; Trio Links require all three.
- Only uninterrupted allied actions advance a Link. Any enemy action breaks the current sequence, and the sequence also clears at round end.
- Relay creates the adjacency needed to complete Links but is not itself a Link component.
- A finishing move that matches a known recipe transforms visibly into the named Link Art before confirmation. The UI previews its new name, participants, and complete changed effect.
- Link recipes are curated. Arbitrary move pairs do not create random effects.
- Encounter rules test broad Link categories such as `any Buff-based Link` or `any Ash Link`, never one exact named recipe required by only one party composition.
- Link Arts are distinct from Weaves: Links are ordered, immediate, allied, and interruptible; Weaves are encounter-authored Echo recipes with their own persistence and resolution rules.

### Battle Chamber acceptance slice

The first vertical slice contains persistent three-Tempo Relay, a small authored duo-Link catalog, and a boss with two intent slots. Shared AP remains a separate Chamber-only move-economy experiment. Successful experiments do not enter expedition combat until their metrics and UI are reviewed.

## Superseded Prototype: Relay Windows

> Historical implementation record only. The rules below are not an active design target and must not govern new work.
...
- Traits, boons, Instincts, Afterforms, items, or encounter rules may explicitly change starting Tempo, the cap, generation, or legal spending, but the base engine assumes none of them do.
...
- A successful action may generate Tempo by exploiting a weakness or authored status interaction. Discovery is not a prerequisite for the Tempo gain.
- A successful conditional critical may generate Tempo.
- Basic Attack is the universal deliberate builder: it sacrifices move potency, and the pack's first Basic hit each round generates one Tempo.
- A move or Instinct may carry another explicit, authored Tempo-generation condition, but learned attacks do not generate merely for landing.
...
#### Conditional spend: Relay Window

- Pack Tempo alone never grants permission to rewrite initiative. Relay additionally requires an open **Relay Window**.
- A conditional critical, weakness exploit, Technical, fully disrupted Omen, Break, or explicitly authored encounter accomplishment may open a Window.
- Basic Attack and other ordinary `on hit` Tempo builders do not open a Window.
- A Window may be spent at most once per round and expires unused at round end.
- The player queues Relay before the current action; if the queued action remains legal when that action resolves, it consumes the Window and one Tempo.
...
- A pulled ally may still generate ordinary Pack Tempo, but cannot open another Relay Window during that round.
- Relay cannot select an ally that already acted, is knocked out, or has no remaining timeline action.
- If every ally has acted, Pack Tempo remains banked but the Window still expires at round end.
- Encounter modules, run-long boons, and Afterforms may explicitly open Windows or add authored alternative Tempo spends.

#### Battle Chamber experiment: shared Action Points

The Battle Chamber currently includes a chamber-only alternative to individual MP so the resource model can be evaluated without silently changing expedition combat:

The full development contract, access instructions, production similarities, and intentional differences are maintained in [`battle-chamber.md`](battle-chamber.md).

- the pack begins each round with three shared Action Points;
- learned moves cost one or two Action Points based on their current MP cost;
- Basic Attack costs zero, so every Kin retains a legal move even after the pool is exhausted;
- unspent Action Points do not bank and the pool refreshes to three at round start;
- Pack Tempo is a separate earned resource and is spent only on Relay;
- spending Action Points can never add, remove, or transfer a creature's one action for the round.

This experiment tests two independent questions: which moves the pack can afford during a round, and whether its move sequence earns a legal Relay. The values and conversion formula are provisional; the separation between the move budget and Pack Tempo is the behavior under test.
...
The initial values are a cap of three, a cost of one Tempo plus one Relay Window, and at most one Tempo generated by each Kin per round. Magnitudes may be revisited through an explicit architecture revision, while action conservation and the one-Window-per-round ceiling are invariants.
...
| Boons | Run modifiers, timed or run-long; usually simple, and any conditional ones must be summarized by the UI | Confirmed as an existing system; future effect scope open |
...
The player should not need to mentally recompute several conditional bonuses before every ordinary action. Traits remain mostly passive and easily summarized. Instincts get one visible trigger per Kin; boons stay simple, and if a run-long boon is conditional the command preview must summarize any currently relevant change. Marks and Afterforms are optional extension points rather than dependencies of the stable core.
...
The current alpha Founding Hand instantiates with Cat carrying Jab/Keening, Geta
carrying Rime/Harden, and Wiggledrake carrying Ashfall/Pyrewood. The Battle Chamber
reads this same authored mapping rather than maintaining a test-only copy. This is a
starter-specific tutorial loadout: wild and bred instances continue to use their
species defaults. When role and archetype move pools replace the current template
system, these pairs must be validated against the one-role/one-archetype rule above.
...
This produces a dependable build cadence tied to the tower's existing boss breaks. Rare Training rewards, events, or run-long boons may grant additional Growth later, but the ordinary post-battle offer is not responsible for delivering the core move system.
...
- a round timeline made of unique action slots with separate slot and actor identities;
- enemy intents keyed by action slot, allowing one boss to advertise two independent threats;
- Pack Tempo generation resolved once per action slot after all qualifying triggers are known;
- a pure Link recipe service shared by command preview and action execution;
...
| CA-02 | Base turn-order model | Confirmed | Interleaved SPD action-slot timeline; one standard slot per combatant per round |
| CA-03 | Enemy-intent granularity | Confirmed | Exact move and target committed and revealed at round start |
| CA-04 | Tempo cap, generation, spending, and persistence | Confirmed | Start 0; cap/cost 3; one base award per complete action; Relay stays Ready until used; Tempo clears after battle |
...
| CA-08 | Responsibilities of Traits, Instincts, boons, Marks, and Afterform | Confirmed boundary | Traits quiet; one Instinct trigger; boons simple and summarized; Marks/Afterforms optional extensions |
...
| CA-11 | Auto-combat and boss-AI behavior | Confirmed | Same rules and information as manual combat; Relay is considered only while Ready and tactically useful |
...
| CA-14 | Player action conservation | Confirmed | One standard action per Kin per round; allied extra action slots are not planned |
...
| CA-40 | Relay readiness | Confirmed | Relay Ready never expires from waiting; spending all 3 Tempo is the default cost |
| CA-41 | Rebound generation | Confirmed | Authored deflection generates Tempo; ordinary resistance does not |
| CA-42 | Link Arts | Confirmed foundation | Curated ordered move-style recipes; different allied participants; enemy actions interrupt; finishing move visibly transforms |
| CA-43 | Multiple enemy actions | Confirmed extension | Bosses may own multiple separately identified and telegraphed action slots in one round |
...
4. **Supporting layers:** Traits, Instincts, boons, Marks, Afterform, items, and auto-combat.
...
The design Q&A remains an evolving architecture record. The 2026-08-01 expiring Relay Window experiment is superseded. On 2026-08-02 the Chamber adopted unique action slots, persistent three-Tempo Relay, authored interruptible Link Arts, and separately telegraphed boss-extra actions; the allied extra-action modifier prototyped alongside them was later removed and is not planned. Shared AP remains a separate Chamber-only move-economy comparison; expedition combat still uses individual MP. Weakness generation is live and does not require prior knowledge, while Omen, Break, and Rebound generation await their combat-event implementations. Weave remains a described encounter module rather than a shortcut inside the core.

---

*Tuning values and content catalogs remain revisable. Action-slot identity, exact intents, visible exceptions, layer responsibilities, and module boundaries require an explicit architecture revision to change.*
