...
- Player Kin receive one action by default. An explicit high-power relic, boon, trait, or encounter rule may grant one Kin a second action; no Kin may act more than twice and only one allied extra action may be created per round.
...
- Explicit relics, traits, boons, and other authored modifiers may increase the amount generated by an action or add new triggers.
...
- **Rebound** means an attack was actively deflected by an authored move, relic, trait, or boon. Ordinary elemental resistance never generates Tempo.
...
- An explicit **Encore Relay** modifier may instead target one Kin that already acted and create a single extra action slot. No Kin may act more than twice in that round.
- Traits, relics, boons, and encounter rules may alter starting Tempo, capacity, generation, persistence between battles, Relay cost, or legal Relay targets. These exceptions must remain visible.
...
The first vertical slice contains persistent three-Tempo Relay, a small authored duo-Link catalog, a boss with two intent slots, and an Encore Relay modifier. Shared AP remains a separate Chamber-only move-economy experiment. Successful experiments do not enter expedition combat until their metrics and UI are reviewed.
...
- Encounter modules, relics, and Afterforms may explicitly open Windows or add authored alternative Tempo spends.
...
| CA-14 | Player action conservation | Revised | One standard action; explicit high-power modifiers may grant one Kin one extra action, capped at twice per Kin and one allied extra slot per round |
