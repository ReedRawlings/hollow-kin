...
| Enemy knowledge | Minimal persistent seen-species memory. Auto exploits weaknesses only on species already met. Blind on first encounter, per `docs/design/combat-system.md`. |
| Tactic roster | The five in `docs/design/combat-system.md`, unchanged. |
...
So Heal First does something genuinely interesting on two creatures, something modest on two more, and on the remaining 32 it degrades straight to its buff, debuff, and damage rules. The ladder is written to degrade gracefully, so this is not a reason to cut the tactic — but it will not feel like it earns its slot until the ability roster grows past its current 31. This is a content problem, not a design problem, and it resolves as abilities are added toward the 72 in `docs/design/Abilities.csv`.
...
- **AUTO toggle** — persistent button in the battle HUD, live during both manual menus and AI turns, so control can be seized mid-fight. A matching toggle in `RunScene`, per `docs/design/combat-system.md`'s "during battle or from the map overview."
...
**Unchanged:** the post-victory reward choice still prompts under AUTO. Auto covers combat only; `docs/design/combat-system.md` is explicit that the player continues to "make strategic decisions between battle."
