...
**Scope:** Re-promote `docs/design/game-design-document.md` to source of truth; reframe the game as permanent-progression rather than roguelite; repair contradictions between topic docs; retire the four-level authority ranking in `CLAUDE.md`.
...
docs/design/game-design-document.md         — SOURCE OF TRUTH for the design
  ├── docs/design/combat-system.md          — owns combat
  ├── docs/design/breeding-and-inheritance.md
  ├── docs/design/tower-structure.md
  ├── docs/design/economy-balancing.md
  ├── docs/design/town.md · marks-*.md · docs/design/traits-system.md · relics · docs/design/ui-ux.md · docs/design/onboarding.md
  └── docs/design/creature-roster-and-generation.md
docs/decisions/         — point-in-time records of decisions, NOT authorities
```
...
## 2. `docs/design/game-design-document.md`
...
Currently four navigation options presented as open. Pick-Next is built and shipped. Rewrite as **"Pick-Next — decided and built"** with a single line noting branching-map / linear-with-rewards / non-linear were considered and rejected. Mirror the same collapse in `docs/design/tower-structure.md` §"Run Shape".
...
| 443 | Delete the open question "Visual and thematic identity for each tower zone." Zones do not exist. The equivalent depth-band question already lives in `docs/design/tower-structure.md`. |
...
Note in the same section that the capture spec cites `capture-mechanics-research`, which is **not in the repo** — treat that spec as self-contained.
...
|---|---|---|
| `docs/design/combat-system.md:29` | Swap action — "replace the active creature with one from inventory (captured creatures mid-run)" | **Delete the Swap action.** It contradicts the capture spec (a capture is cargo, arrives at level 1, cannot be fielded during the run that caught it) and it does not exist in code — `CombatScene` offers abilities, basic attack, and defend only. The Capture action on the next line is correct and stays. |
| `docs/design/game-design-document.md:115` and `docs/design/tower-structure.md:88` | Rest points / pick-next let the player "swap party members in and out of the active roster" | Same root cause. The player brings exactly three creatures and captures are cargo, so there is no reserve to swap from — the feature has no contents. Remove the swap clause from both; rest points restore HP/MP and can teach an ability. Revisit only if a reserve party is ever designed. |
| `docs/design/onboarding.md:35` | Breeding — "marks optionally carry" | Marks are **never** inherited through breeding (`docs/design/marks-system.md`, GDD). Remove the clause. |
| `docs/design/onboarding.md:1` | Title reads "Hollow Kin — UI/UX & Player Information Design" | Copy-paste from `docs/design/ui-ux.md`. Retitle to "Hollow Kin — Onboarding" and rewrite the Overview paragraph, which is also `docs/design/ui-ux.md`'s. |
| `docs/design/ui-ux.md:65` | Breeding screen — "Optional: Breeding Relic or Stone slot" | Both cut. Remove the line. |
| `docs/design/ui-ux.md:133` | Visual Language table — "Stones · Rough gem shape · Blue" | Remove the row. |
| `docs/design/ui-ux.md:90` | Post-Encounter — "XP gained and level-up notifications" | Correct but ambiguous under Model A. Clarify that these are temporary in-run levels and cross-reference the persists/resets table. |
...
Trait progression is contradictory across four documents, and the contradiction is **not** a wording problem. `docs/design/traits-system.md` says slots and levels unlock at essence thresholds paid at the Trait-keeper (in town). `docs/design/breeding-and-inheritance.md` says a slot unlocks when a creature hits its level cap, and that inherited traits resolve "at run time." Those describe two different systems with two different resolution moments — town-at-purchase versus in-run-at-cap. Choosing between them is a design decision with real consequences for when the player sees a trait roll, so it belongs in its own conversation rather than in a cleanup pass.

**Left untouched, deliberately:**

- `docs/design/breeding-and-inheritance.md:35` — trait slot unlocked by hitting the level cap
- `docs/design/breeding-and-inheritance.md:84–94` — Trait Resolution Cases 2/3/4 resolving at run time on hitting the star's level cap
- `docs/design/traits-system.md:138–140` — inheritance summary restating the star/run-time model
- `docs/design/creature-roster-and-generation.md:153` — natural trait pool rolling "when a wild creature earns a new star"
- `docs/design/marks-system.md:7` — "traits are inherited through bloodlines and strengthen over stars"
- GDD "Trait Inheritance" and "Traits and Essence" sections, beyond the Stone clause removal in §2.6

**Handling in the interim.** Rather than leaving these silently wrong, `docs/design/traits-system.md`, `docs/design/breeding-and-inheritance.md`, and the GDD's trait sections get a visible header:

> ⚠️ **Trait progression is under active revision (as of 2026-07-26).** This document and `docs/design/breeding-and-inheritance.md` describe two incompatible unlock models — essence thresholds paid in town versus level-cap unlocks resolved in-run. Do not implement trait unlock or trait resolution from these docs until the open design question is settled.
...
> The GDD is the design source of truth. Topic docs own their own subjects. Specs in `docs/decisions/` record how decisions were reached; they do not override the GDD.
>
> **One open exception:** trait progression is under revision — `docs/design/traits-system.md` and `docs/design/breeding-and-inheritance.md` disagree on how trait slots unlock. Do not implement traits from either until that is settled.
...
- `relics` — correctly run-scoped. Relics resetting is the design, not a leftover.
- `docs/archive/retired/breeding-stones.md` — already carries a clear RETIRED/CUT banner.
- `docs/design/economy-balancing.md`, `docs/design/town.md`, `docs/design/tower-structure.md` — fully converted to the essence model, apart from the Run Shape collapse in `docs/design/tower-structure.md` (§2.3).
- `docs/design/Abilities.csv` — data, not prose.
