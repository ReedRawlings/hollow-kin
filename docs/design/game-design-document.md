...
* Essence (permanent) is spent on levels, trait unlocks, depth-jumps, and backpack capacity. Marks cost nothing — they are discovered, never bought. Obols (in-run) are spent on heals, revives, capture, and shop items.
* Persistent progress lives in **essence investment and the breeding genealogy** — bloodlines, stars (for now), and discovered marks
...
Progression is permanent. A handful of things are still deliberately run-scoped, and **those three are the ones that get mistaken for leftovers from the old roguelite design.** Each is intentional. Do not "fix" them into persistence.
...
Everything else persists: Essence, permanent levels, stars, bloodline, discovered marks (permanent the instant they are discovered — no binding step), unlocked trait slots, purchased depth-jumps, backpack capacity, and creatures themselves.
...
* **Permanent and non-refundable** — once spent on a creature, essence is locked to it. Spent on permanent levels, trait unlocks, depth-jumps, backpack capacity
...
* Discover **Marks** by doing unusual, possibly hidden deeds — each one permanently unlocks new content (a boon, an item, a trait in stock, a title, an event)  
...
| `spd` | Turn order only — crits are authored per-ability `critCondition`s, never a SPD roll. **Not** evasion — see the Resolved Design Decisions note below |
...
| *(marks)* | Not a creature field. Marks are player-level discoveries (`discoveredMarks` on the save, each recording the accomplishing Kin and run). ⚠️ **Not in code** — no mark system exists in any form |
...
> **Decided 2026-08-27, not built.** Marks are permanent *discoveries*. The older
> equipped-bonus model was retired the same day; `marks-system.md` is the authority and
> carries the starter deed table. Building it waits on capture and boss variety. No code
> references marks in any form.

Where marks sit among the other layers:
...
* **Discovered** by doing a deed during a run — deeds may be hidden, with clues in names, Monsterpedia entries and event text  
* **Permanent immediately.** No Essence, no slot, no equip, no binding step; a mark cannot be lost, not even on a wipe  
* **Not a creature stat bonus.** The reward is content entering the game: a new boon in post-battle offers, a new item in shops and rewards, a rare trait in the Trait-keeper's stock, a title or lineage epithet, or a new event, breeding option or boss rematch  
* **Recorded** against the accomplishing Kin and the run it happened on  
* Deeds anchor on the boss cadence (mini-boss every 5, major every 10), never on a floor number, so the deed set extends with the tower  

### **Example Deeds**

* **Kindred Victory** — defeat a boss with all three Kin sharing an archetype; unlocks a party-affinity run-long boon  
* **Last Light** — win three battles in one run while the same Kin stays below 20% HP; unlocks a run-long boon that lets its holder survive lethal damage at 1 HP once per run  
* **Temper the Ash** — fell three Ash-dealing enemies with Salt damage in one run; adds a Salt-ward phial to shops and rewards  

### **Marks and Breeding**

* Marks are not on the creature, so there is nothing to inherit — the unlocked content belongs to the player and survives retirement
* What passes down is the **record**: a Kin's discovered deeds stay in its history and can show in its descendants' lineage; titles are worn by the line
* The **Ledger** in town lists discovered deeds in full and undiscovered ones as clues — see `marks-system.md` and `town.md`
...
Full capture design — duplicate Essence grant, box capacity, pending-capture queue — is specified in `docs/decisions/2026-07-25-capture-system-design.md`. That spec predates the rite/band-price model above and describes an earlier threshold model; where the two disagree, **`src/systems/Capture.ts` is the authority.**
...
| **The Ledger** | Read discovered marks and clues to undiscovered ones | No — reading only |
...
* Progression is **permanent and essence-driven** (2026-07-23 pivot) — creatures no longer reset to level 1; they keep an essence-bought level floor. Plasm, Breeding Stones, and Longevity are removed. See `docs/decisions/2026-07-23-essence-progression-pivot-design.md`.
...
* Marks are **permanent discoveries that unlock content** (2026-08-27) — the earlier equipped-bonus, Essence-bound model is retired; see `marks-system.md`. docs/archive/retired/breeding-stones.md is retired/cut
...
* Accuracy — uses the Accuracy column from the ability CSV. SPD does not affect evasion; it handles turn order only (crits are conditional per ability, not rolled). ⚠️ **Evasion does not exist in code at all** — there is no target term in the hit roll, so the Evasion Up trait and the Blind status both have no effect. See `combat-system.md`
* Buff/debuff stages — capped at ±3, ranging from 0.75x to 1.5x. Tighter than Pokémon's system to prevent snowballing
* Critical hits — **conditional, not rolled**: a hit crits when the ability's authored `critCondition` holds (target statused / debuffed / below half HP), never from a random roll or an SPD term, at 1.5x damage. **Enemies crit too** (decided 2026-08-27) — the old player-only rule existed to keep run-ending random crits off the enemy side, but conditional crits aren't a roll, so that rationale no longer applies. ⚠️ The "ignores the target's defensive buffs" clause was specified and **never built** — `calculateDamage` crits off the target's buffed stats. See `combat-system.md`
...
How the docs relate and which wins is in `docs/README.md`. Short form: specs in `docs/decisions/` record **how a decision was reached** and are not authorities — **this document is the source of truth.** Where a spec and the GDD disagree, that is a bug in the GDD to be fixed.

| Spec | What it decided | Built? |
| ----- | ----- | ----- |
| `docs/decisions/2026-07-23-essence-progression-pivot-design.md` | The permanent-progression model itself — Obols→Essence, permanent level floors, 30-floor descent, essence-hub town | Yes (Phases 1–4a) |
| `docs/decisions/2026-07-24-auto-combat-tactics-design.md` | Tactic ladders, knowledge fog, persisted battle speed | Yes |
| `docs/decisions/2026-07-25-departure-flow-design.md` | Standing default party, pre-run departure screen | Yes |
| `docs/decisions/2026-07-25-capture-system-design.md` | Capture threshold model, duplicate Essence grant, box capacity, pending-capture queue | **Designed only** |
| `docs/decisions/2026-07-25-monsterpedia-design.md` | Bestiary UI over `gameState.seenSpecies` | Yes — `BestiaryScene`, reachable from THE ARCHIVE |
| `docs/decisions/2026-07-26-doc-realignment-design.md` | Documentation pass — GDD re-promotion and contradiction sweep | — |
| `docs/decisions/2026-07-26-traits-system-design.md` | Trait slots by permanent level, traits found-and-imbued, Trait-keeper's role, stars kept | **Designed only** |
| `docs/decisions/2026-07-29-expedition-commitment-and-consumables-design.md` | Departure earned at boss floors or bought as a Waystone; the item pool 2 → 9; the run-map bag made usable | Yes |
| *(no spec — design agreed in conversation; rationale inline in* `docs/archive/plans/2026-07-29-post-battle-offers-and-boons.md`*)* | The post-battle three-card reward offer, and **timed boons** replacing the pitch's charged Preparations | Yes |

> **Note:** the capture spec cites a *capture-mechanics-research* survey, which is **not in this repo**. Treat that spec as self-contained.
