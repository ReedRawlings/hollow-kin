...
- Point-in-time records under `docs/decisions/` and the retired `docs/archive/retired/breeding-stones.md` are deliberately left as written — they are history, not guidance.
- Added `docs/dev/level-calculator.xlsx`: an exact in-run level model (per-encounter log + expected-value scenario) built from `Battle.ts` XP rules and `GameState.tryLevelUp`; verified against the game loop. Finding: a full 1→20 descent yields ~296 XP → level 7 from level 1, so a 0★ starter caps at 5 by mid-run.
...
- **Marks adopt Design B — permanent discoveries** (decided 2026-08-27). The owner's taxonomy: boons are run-scoped modifiers; consumables answer the run's dangers; traits define what a creature is; marks are the player's recorded accomplishments that unlock content. A mark is discovered by a deed, permanent at once, no Essence, no slot, no creature stat bonus; its reward is new content (boons, items, traits, titles, events, rematches). Design A (earn-then-lock, equipped percentages) is retired; `marks-catalog` deleted. Not built — waits on capture and boss variety.
- **Heirlooms are cut**, not paused. A permanent equippable would have been a third per-creature slot system beside Traits and Marks, and its example effects were free once-per-run consumables. If one ever returns, its runtime effect is a run-long boon granted at departure; only the collection and equip UI would be new.
- **Docs reorganised** (2026-08-27). Root keeps only `CLAUDE.md`, `AGENTS.md`, `progress.md`. `docs/design/` is the live design (GDD + topic docs); `docs/decisions/` the dated specs; `docs/archive/` executed plans, pitches, research and retired docs; `docs/dev/` the Battle Chamber, Phaser Editor notes and the level calculator. Precedence: code → CLAUDE.md → docs/design → docs/decisions → docs/archive. The combat architecture spec (`docs/combat-architecture-spec`) was reconciled into `docs/design/combat-system.md` and archived. `output/`, `test-actions/` and `_to_delete/` are deleted; `output/` and `test-actions/` are now gitignored.
- **Combat open questions resolved** (2026-08-27): buff/debuff stages stay whole-battle and stacking (±3 cap, Null Salt as the counter) — a deliberate alpha choice, revisit with playtest data; **enemies crit** on the same authored `critCondition`s as players — crits are telegraphed, not rolled, so the old player-only RNG rationale no longer applies; **enemy count is 1–3 in bands 1–2 and up to 5 from band 3**; **Pack Tempo keeps its single source** (weakness hits) until Relay is validated as a combat system — Omen/Break/Rebound stay the intended future sources, and the UI copy promising them is removed. Still open and playtest-bound: damage constants, Fight Wisely's MP drain, Heal First's reserve.
